...
//스크롤이 100보다 작거나 같을때 nav에 mouseover하면 컬러 mouseleave하면 흰색
$(window).scroll(function () {
  if ($(this).scrollTop() <= 100) {
    $('nav').mouseover(function () {
      $('.gnb li a').css({
  ...
  //그렇지 않고, nav에 mouseover하면 컬러 mouseleave하면 컬러
  else {
    $('nav').mouseover(function () {
      $('.gnb li a').css({
        'color': '#000'
...
$('.feed1, .feed2, .feed3, .feed4, .feed5, .feed6, .feed7, .feed8, .feed9, .feed10, .feed11, .feed12').click(function () {
  ...
  // Get the number from the clicked element's class
  var feedNumber = $(this).attr('class').match(/\d+/)[0];

  // Show the corresponding pop_up_feed element
  $('.pop_up_feed' + feedNumber).show();

  // Show the pop_up_wrap element
  $('#pop_up_wrap').show();

  // Add the scroll_none class to the body
